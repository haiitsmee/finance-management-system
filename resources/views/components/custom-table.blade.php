...
@props([
    ...
    'divisi' => $params['divisi'] ?? null,
])
<label for="{{ $id }}" class="py-4">{{$label}}</label>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        @if($hasDetail)
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                <a href="{{ route($href, ['divisi'=>$divisi, 'id' => $item['id']]) }}" class="bg-gray-800 rounded-md text-white px-3 py-1 hover:bg-gray-500">
                                    Detail
                                </a>
                            </td>
                        @if($hasAction)
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                ...
                                <x-table.button-table :submit="false"
                                    redirect="{{ route($routeName, ['divisi'=>$divisi, 'id' => $item['id']]) }}" color="" value="Edit"
                                    type="edit" />
