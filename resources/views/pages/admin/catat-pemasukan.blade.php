...
@section('content')
    <div class="flex flex-col">
        <h1 class="text-2xl text-black">Catat Pemasukan Divisi: <strong>{{ session('current_business_name') }}</strong></h1>
        ...
        <div class="flex min-w-full justify-start gap-8 px-4 py-5 md:flex-col flex-row">
            <form action="{{ route($role . '.' . 'catat-pemasukan.store', ['businesses' => session('current_business')]) }}" method="POST" enctype="multipart/form-data">
            @csrf    
            <div class="flex min-w-full justify-start gap-3 lg:gap-8 px-4 py-5 md:flex-row flex-col" >
                    ...
                    <div class="flex flex-col gap-3">
                        <x-form.input for="business" label="Divisi Terkait" placeholder="Divisi Terkait" :required="true" :readonly="true"
                            type="text" :value="session('current_business_name')" width="w-70" />
                    ...
                    <x-forms.file-input for="gambar" label="Bukti Pendapatan" :required="true" width="w-70" note="Hanya mendukung format JPG, JPEG, PNG" />
                    </div>
            </div>
        ...
        <div class="mt-10">
            <div class="flex self-end gap-3 justify-end">
                <form id="search-form" action="{{ route($role . '.' . 'catat-pemasukan.index', ['businesses' => session('current_business')]) }}" method="GET" class="flex items-center gap-1">
            ...
            <x-table.data-table>
                <x-slot:header>
                    ...
                    <th class="px-6 py-3 text-sm font-medium text-custom-light-gray tracking-wider">Total Pemasukan</th>
                ...
                <x-slot:row>
                    @foreach ($transactionIncomes as $income)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <x-table.button-detail redirect="{{ route($role . '.' . 'catat-pemasukan.show', ['businesses' => session('current_business'), $income->id ]) }}" />
                            </td>
                        </tr>
                    @endforeach
                </x-slot:row>
            ...
            <div class="mt-3">
                {{ $transactionIncomes->links('pagination::tailwind') }}
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('tanggal').addEventListener('change', function() {
            ...
            if (tanggal) {
                window.location.href = "{{ route($role . '.' . 'catat-pemasukan.index', ['businesses' => session('current_business')]) }}" + "?tanggal=" + tanggal;
            } else {
                    window.location.href = "{{ route($role . '.' . 'catat-pemasukan.index', ['businesses' => session('current_business')]) }}";
            }
        });
