...
@section('content')
    <div class="flex flex-col">
        <h1 class="text-2xl text-black">Catat Pengeluaran Divisi: <strong>{{ session('current_business_name') }}</strong></h1>
        ...
        <div class="flex min-w-full justify-start gap-8 px-4 py-5 md:flex-col flex-row">
            <form action="{{ route(auth()->user()->role . '.catat-pengeluaran.store', ['businesses' => session('current_business')]) }}" method="POST"   enctype="multipart/form-data">
            @csrf    
        ...
        <div class="mt-10">
            <div class="flex self-end gap-3 justify-end">
                <form id="search-form" action="{{ route(auth()->user()->role . '.catat-pengeluaran.index', ['businesses' => session('current_business')]) }}" method="GET" class="flex items-center gap-1">
                ...
                <x-slot:row>
                    @foreach ($transactionExpenses as $income)
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">@currency($income->price)</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <x-table.button-detail redirect="{{ route(auth()->user()->role . '.catat-pengeluaran.show', ['businesses' => session('current_business'), $income->id ]) }}" />
                            </td>
                        </tr>
                    @endforeach
                </x-slot:row>
    ...
    <script>
        ...
        document.getElementById('tanggal').addEventListener('change', function() {
            ...
            if (tanggal) {
                window.location.href = "{{ route(auth()->user()->role . '.catat-pengeluaran.index', ['businesses' => session('current_business')]) }}" + "?tanggal=" + tanggal;
            } else {
                    window.location.href = "{{ route(auth()->user()->role . '.catat-pengeluaran.index', ['businesses' => session('current_business')]) }}";
            }
        });
