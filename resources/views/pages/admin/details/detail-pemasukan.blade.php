...
@section('content')
    <div class="flex flex-col">
        <h1 class="text-2xl text-black">Detail Pemasukan Divisi: <strong>{{ session('current_business_name') }}</strong>
        </h1>
        ...
        <div class="flex flex-col gap-2">
            ...
            @if ($fileExists)
                <div class="w-1/3 h-auto border border-black flex items-center justify-center">
                    <img src="{{ asset('storage/' . $transactionIncome->image) }}" alt="Bukti Pembayaran">
                </div>
            @else
        ...
        <div class="flex justify-end">
            <a href="{{ route(auth()->user()->role . '.catat-pemasukan.cetak-pdf', ['businesses' => session('current_business'), 'id' => $transactionIncome->id]) }}" type="button" class="flex w-max h-10 p-4 mt-7 items-center rounded-md border bg-[#050A30] text-white text-md self-end hover:bg-gray-200 hover:text-[#050A30]"><span>Cetak PDF</span>
            </a>
        </div>
    </div>
